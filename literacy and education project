...
-- Sources:
	-- Patrinos and Angrist (2018) and UNESCO via World Bank; Data compiled from multiple sources by World Bank 
	-- World Bank (2023), Various sources (2018) – OurWorldInData.org/literacy
...
USE education;
DROP TABLE IF EXISTS literacy_vs_avg_ed;
...
SELECT * FROM literacy_vs_avg_ed;
...
SELECT 
    ...
    DENSE_RANK() OVER(ORDER BY male_youth_literacy) AS m_lit_rank,
	male_youth_literacy,
    DENSE_RANK() OVER(ORDER BY female_youth_literacy) AS f_lit_rank,
    female_youth_literacy
FROM
	literacy_rates_male_female
WHERE year = 2011 AND code != ' '
;
...
SELECT * FROM f_data_years;
 ...
 SELECT 
	DISTINCT(adj.country),       # this was done because the original query resulted in multiple repetitions for each country
    adj.year,
...
FROM
	adjusted_school_years adj
		JOIN
	tertiary_education te ON adj.country = te.country
